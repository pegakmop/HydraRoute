# Архитектура Hydra Route Neo - Принцип работы и оптимизации

## 1. Общий обзор системы

HR Neo - это система маршрутизации DNS-трафика через туннель на основе списка доменов (watchlist). Программа перехватывает DNS-запросы, анализирует их и автоматически добавляет IP-адреса целевых доменов в ipset, после чего iptables правила направляют трафик через туннель.

### Основная архитектура

```
Клиент → DNS-запрос → NFLOG (копия пакета) → HR Neo анализирует
                ↓                                      ↓
        Пакет идёт дальше                    Сохраняет txID в кеше
                ↓                                      ↓
        DNS-ответ → NFLOG (копия) → HR Neo → Добавляет IP в ipset → Разрывает conntrack
                ↓                                                            ↓
        Клиент получает ответ                              Новое соединение через туннель
```
...
### Старая релизная версия

**Проблемы архитектуры:**

Старая версия представляла собой единый файл размером более 900 строк кода.  
Все компоненты системы были разрознены:
...
Такой подход приводил к проблемам с производительностью.
...
```
- Главный файл, инициализация, обработка сигналов
- Перехват DNS через NFLOG (ядро Linux)
- Низкоуровневая работа с ipset через netlink
- Высокоуровневый интерфейс для ipset операций
- Управление conntrack (разрыв соединений)
- Вспомогательные функции (config, iptables, etc)
- Определение архитектуры и byte order
```
...
Программа HR Neo получает копию пакета через netlink socket и обрабатывает её в отдельной горутине. Вся обработка (парсинг DNS, поиск в watchlist, добавление в ipset, разрыв conntrack) происходит параллельно с доставкой оригинального пакета к получателю.
...
Клиент получает DNS-ответ с той же скоростью, как если бы HR Neo не был установлен. Вся обработка происходит в фоновом режиме и никак не влияет на латентность сети.
...
HR t=5.0ms  (общее время обработки HR Neo)
```
...
HR t=0.8ms (общее время обработки HR Neo)
          
✅ Существующие TCP соединения продолжают работать без разрывов!
✅ HR Neo обработал за 0.8ms (без разрыва conntrack)
```
...
✅ HR Neo обработал новый IP за 1.2ms (с разрывом conntrack)
```
...
- **HR t=** - время работы HR Neo

**Ключевые моменты:**

1. **Первое добавление IP**: HR Neo работает 5.0ms (включая разрыв conntrack)
2. **Повторный DNS-запрос**: HR Neo работает 0.8ms (БЕЗ разрыва conntrack)
3. **Смена IP**: HR Neo работает 1.2ms (с разрывом conntrack для нового IP)
...
**Общий вывод:** Переход от архитектуры на CLI к модульной с netlink обеспечил многократный прирост производительности и стабильность соединений, обеспечив при этом фактчиески нулевую задержку для пользователя.  
Ключевое улучшение - Низкоуровневая работа с ipset через netlink.
