...
const Utils = {
    ...
    consolidateDomains(domains) {
        ...
        domains.forEach(domain => {
            ...
            if (cleaned) {
                const root = this.getRootDomain(cleaned);
                rootDomains.add(root);
            }
        });
        return Array.from(rootDomains);
    }
};
...
const DomainManager = {
    ...
    async loadFromGithub(serviceUrls) {
        ...
        return Array.from(newRootDomains);
    }
};
...
const Validator = {
    ...
    findDomainConflicts(domains1, domains2) {
        ...
        domains1.forEach(domain1 => {
            domains2.forEach(domain2 => {
                if (Utils.getRootDomain(domain1) === Utils.getRootDomain(domain2)) {
    ...
    showValidationWarning(domainErrors, activePolicies) {
        ...
        tableHTML += '</table>';
        tableHTML += '<div style="margin-bottom: 15px; color: #666;">Обнаружены пересекающиеся домены. Это может привести к непредсказуемому поведению маршрутизации.</div>';
